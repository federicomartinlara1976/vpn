package net.bounceme.chronos.vpn.server;
...
public class Server implements CryptoInterface {
	...
	/*
	 * protected void startConnection() { m_gui.connectionReady(); }
	 */
	...
	public void acceptConnection() {
		...
		try {
			...
			// Generate Diffie-Hellman keypair, and send data to client to continue
			// handshake.
			...
			byte[] encryptedServerIdentification = encryptHandshake(sharedSecretKey, SERVER_IDENTIFICATION.getBytes(),
					Cipher.ENCRYPT_MODE);
			...
			writeToClient(outputStream,
					encryptHandshake(sharedSecretKey, clientChallenge.getBytes(), Cipher.ENCRYPT_MODE));
			...
			String serverChallengeReturned = new String(
					encryptHandshake(sharedSecretKey, serverChallengeData, Cipher.DECRYPT_MODE));
			if (!serverChallengeReturned.equals(serverChallenge)) {
				m_gui.printf("Incorrect server challenge returned, terminating program");
				m_gui.connectionClosed();
			}
			byte[] unencryptedEncodedPublicKeyBytes = encryptHandshake(sharedSecretKey, clientPublicKeyBytesEncoded,
					Cipher.DECRYPT_MODE);
			...
			// Handshake is done, now encrypt all user input in DES format using shared
			// secret key.
			diffieHellmanHelper.getKeyAgreement().doPhase(clientPublicKey, true);
	 ...
	 * 
	 * String userInput; while (true) { byte[] clientInput =
	 * readFromClient(inputStream); String clientInputDecrypted = new
	 * String(communicationDecryptionCipher.doFinal(clientInput));
	 * System.out.println(clientId + " sent: " + clientInputDecrypted);
	 * System.out.println(">>"); userInput = userInputReader.readLine();
	 * writeToClient(outputStream,
	 * communicationEncryptionCipher.doFinal(userInput.getBytes())); }
	 * 
	 * }
	 */
	...
	private byte[] encryptHandshake(String sharedSecretKey, byte[] plainText, int encryption_mode)
	...
	private Cipher getCommunicationCipher(int encryption_mode, SecretKey secretKey)
