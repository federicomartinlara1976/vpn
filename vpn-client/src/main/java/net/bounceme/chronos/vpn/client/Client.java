...
//			inFromServer.read(serverPublicKeyEncoded);
			...
			m_gui.printf(
					"Connection to " + serverIdentification + " has been established. Completing handshake protocol..");

			// instantiate a DH public key from the encoded key material and get key
			// parameters.
	...
	public byte[] encryptDES(SecretKey k, byte[] plaintext) throws NoSuchAlgorithmException, NoSuchPaddingException,
	...
	public byte[] decryptDES(SecretKey k, byte[] plaintext) throws NoSuchAlgorithmException, NoSuchPaddingException,
			InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		// Create PBE Cipher
		Cipher desCipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
		// Initialize PBE Cipher with key and parameters
	...
	public byte[] encryptHandshake(String sharedSecretKey, byte[] plainText, int encryption_mode)
